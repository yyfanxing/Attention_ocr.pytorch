...
def val(encoder, decoder, criterion, batchsize, global_step, dataset, teach_forcing=False, max_iter=100):
    print('Start val')
    ...
    for i in range(max_iter):
        ...
        # stuff['s', 'o', 'n', 'e', 'r', 'e', '<EOS>']
        # if cpu_texts[0] == decoded_words:
        decoded_words = [x for x in decoded_words if x!='<EOS>']
        predict = ''.join(decoded_words)
        gt = cpu_texts[0]
        if predict == gt:
            n_correct += 1
            # print('pred:%-20s, gt: %-20s' % (predict, gt))

    accuracy = n_correct / len(data_loader)
    # accuracy = n_correct / float(n_total)
    writer.add_scalar('test/Loss', loss_avg.val(), global_step)
...
if __name__ == '__main__':
    ...
    t0 = time.time()
    global_step = epoch_start_index * len(train_loader) + batch_start_index
    for epoch in range(opt.niter):
        train_iter = iter(train_loader)

        for batch in range(batch_start_index, len(train_loader)):
            global_step += 1
            ...
            if global_step % opt.displayInterval == 0:
                print('[%d/%d][%d/%d] Loss: %f Lr: %.6f' %
                    (epoch, opt.niter, batch, len(train_loader), loss_avg.val(), opt.lr), end=' ')
            ...
            if global_step % opt.valInterval == 0:
                val(encoder, decoder, criterion, 1, global_step, dataset=test_dataset, teach_forcing=False)

            if global_step % opt.saveInterval == 0:
                torch.save(
                    encoder.state_dict(), '{0}/encoder_{1}.pth'.format(opt.experiment, global_step))
                torch.save(
                    decoder.state_dict(), '{0}/decoder_{1}.pth'.format(opt.experiment, global_step))